...
所以在优化的过程当中，我们将XPath从主路径降级为兜底，在进行元素定位时优先尝试通过 Set-of-Mark 标注中设置好的 ID 进行配对（ID在结构块和属性里面 不受DOM树变化的影响），然后如果没法定位到元素则降级为XPath计算处理，还是无法定位元素则用CSS选择器检查该类元素是否存在，没有则报错。
