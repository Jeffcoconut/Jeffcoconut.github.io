...
```
如果根据硬划分条件会被划成3个框，然后每个框被赋予一个编号。但是从外部视觉角度而言，这三个元素应当同属一个交互框内，分成三个紧凑且重叠的框只会影响模型识别效率以及人外部审计检查。所以有必要将视觉上联通重叠的兄弟框合并成一个大的方框。
...
最后AI给出的解决方案是通过DOM计算来为每个网页计算一个指纹。每个页面的指纹由以下四个信号组成：
| 信号 | 捕捉什么 |
|---|---
|总文本长度 | 内容整体增减 |
| 列表项数 | 滚动追加（项数变多） |
