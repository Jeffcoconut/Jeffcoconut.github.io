...
**mini_skyvern project**是我首次尝试做一个中大型的项目，整个复现+复盘总结经历的时间跨度比较长，在此期间也经历了一些方向上的调整和转变。

原**mini_skyvern project**仅规划了一篇技术复盘文章,但是在不断的修改、调整以及优化过程中，笔者发现一篇文章远远不够，遂临时扩展成三篇文章。由于是临时调整+初次写大项目技术文档复盘，所以三篇文章之间的架构略有些混乱。我已尽力尝试调整，如果有语言逻辑上不通顺之处，请海涵。

作为写作顺序的第一篇文章，我先简单介绍一下三份复盘文章的内容和结构，以便读者进行阅读。
...
>不过在做完项目以后，我并不认为快速跑出一个小demo是一个很好的idea了，后续调整优化的工作量几乎是跑一个小demo的数倍。相关感想可以看8月份的总结反思文章。
{: .prompt-warning}
...
为了让LLM尽可能拿到核心的"正文"内容，我们进一步优化了 Set-of-Mark 的页面标注以及元素提取优先级。
...
>直白而言就是如果父级是内容容器这种冗余的东西，那么就将父亲元素删掉。跟第二条进行的处理略有些相似，可以理解为进一步的处理。

而在元素收集的时候，进行父块子块去重还是考虑到token限制的问题。百度通过父子块设计的目的是为了提升浏览器用户的使用体验，但是本质上父子指向的网址是同一个网址。把这样的内容直接塞入LLM会导致紧张的token限额被重复上下文所挤占，而且后续LLM在读取选择交互元素的时候还需要再画时间进行决断，影响效率。

所以在满足 **（父子俩都在收集列表里）&&（（子块被父块盖住80%）||（子 ∈ {a, button} 且 父 ∈ {div, span, li, p, section, article}））** 的情况下就会把删除父块保证准确性与效率。

**然后，进行分组合并元素**（兄弟元素之间）。因为通过 DOM 切分页面将页面变成了极小的元素，比如下文的一个带图标的按钮：
```html
<button>              <-元素 1:按钮本体
    <svg>...</svg>    <-元素 2:放大镜图标
    搜索               <-元素 3:文字（可能被 span 包着）
</button>
```
如果根据硬划分条件会被划成3个框，然后每个框被赋予一个编号。但是从人类外部视觉角度而言，这三个元素应当同属一个交互框内，分成三个紧凑且重叠的框只会影响模型识别效率以及人外部审计检查。所以有必要将视觉上联通重叠的兄弟框合并成一个大的方框。

在处理联通的方框重叠问题上，AI 进行了巧妙地转换，把页面简化为了图，然后把原问题转化为“图的连通分量问题”。转化方式如下图所示：

```
图论模型                     页面上的对应物
──────────────────────────────────────────
顶点（vertex）        =      一个带 unique_id 的元素（框）
边（edge）            =      两个框「几何上相交」
连通分量（component） =      一个视觉分组（画一个合并大框）
```

在旧版 mini_skyvern 的BFS合并当中，只要两个元素边框之间有重合就会被合并到一起，而且单个集合内的元素合并量没有上限限制。最后不同元素框间越合并越大，甚至可能被合成一个大的边框。

在此处 AI 的解决手法比较直接，采取了一个简单的贪心，即拿到一个元素后，只能将其一跳以内的元素框来进来合并，然后已经合并的元素框就不再作为始发的探索点。

虽然这样贪心可能会将不相同的内容块给合并到一个框中，然后本来应合并到一起的被分到了的框中。但是这产生的问题集中在框框辅助标记这块，对于链接本身的数据层没有影响，（可能需要让LLM自行多判定一下）相当于牺牲了一定的辅助边框准确度换整体的识别度。

然后，在进行不同框之间的相交与否判定时，如果两两比较就是一个 O(n²) 的算法，但是 AI 在此处采取了一个类似二维空间的"二分 + 剪枝"的四叉树空间索引算法。（~~不太清楚这个算法是不是AI自己命名出来的~~）

>（~~此处本来应该有算法流程图的，但是让AI生成了一下感觉太长了所以改为我自然语言描述~~）

大体的思路就是将一个空间页面分为4份，然后判定每个小空间当中的元素个数。如果元素个数大于阈值（`maxElements=10`）则继续进行四分，如果小于则停止四分。然后最后做查询判别的时候只在自己的小的空间子块里面连通。

对于横跨多个空间的元素，则不下沉留在父空间里面。等其他元素判定完以后再一同进行连通判定。

> 通过大量的总结我们其实也能发现，AI 在prompt指令要求不够细致的情况下，对于某一个具体流程不会一下子做单步的最优化，但是会通过多层测分级处理做多步的鲁棒优化，最后达成的效果也相当不错。但是如果能提更详细的算法优化要求，应该也是能够在性能上达到更好
{: .prompt-tip}

**最后，对不同元素进行配额加权**。

虽然进行了 token 预算管理，但是如果顶到了预算上限则需要去决定放弃那些元素，保留哪些元素。AI 给出了一套打分标准，按照分数保留需要的元素。

具体加分规则如下：
```plaintext
+1000 分：元素文本或链接地址里，含有「任务目标的关键词」
+100  分：元素是核心交互标签（a / button / input / select / textarea / 下拉控件）
```
然后分数从高到低进行排序，最后将提取的元素量限制到阈值范围以下。

>至此，我们完成了对于网页上可交互元素的选择与处理，随后对相关元素列表进行标准化处理以后最终的查处流向决策模型，并进行动作执行。
{: .prompt-info}

#### 列表选择器：全打分择优

打分选择容器是与可交互元素收集并行的一条业务逻辑线，主要目的是为了满足数据提取结构化相关的的逻辑操作。

两条线在基础设施层面是共享的：
- 都通过`page.evaluate`把JS注入浏览器当中，在页面进程里操作 DOM；
- 都遵守同一套 token 配额纪律（元素列表砍到600条，列表项60条）
- 都在 Agent 主循环的每一步触发。

我们不妨先看一下旧版 mini_skyvern 的数据提取链路。

旧版候选选择器列表是一个有序兜底链：

```python
_LIST_SELECTORS = [
    ".result", ".item", ".list-item", ".card", "article",
    "[data-testid*=result]", ".search-result", ".news-item",
    ".product", ".product_pod", "tr", "li",
]
```

逻辑也很简单，就是从上往下试，**谁先命中就用谁**。但是这存在一个问题，即前面的如果没有抓到合适的数据结构， `li`几乎在任何页面都能命中——包括导航栏和页角。一旦真正的内容容器没有配上排在前面的11个选择器，`li` 就会把导航菜单当数据抓回来。（~~除非在这个list中按照很严谨的规则进行穷举~~）

所以在新版当中，直接把所有的候选选择器全部进行打分然后取最高，并放在一段JS里面一次性算完。（这样就只要调用一次CDP）

在对容器设计打分标准的时候，主要考虑了两个维度：
- 其一是 C（n）数量维度。n是这个选择器在页面上匹配出的节点个数。如果数据列表条数太少则不构成列表，如果选的太多则说明选的过于宽泛。
- 其二是 L̃ 质量维度。L̃ 是所有命中节点文本长度的中位数。我们希望截取匹配出来的是有一定内容的数据，而不是大量的短文本形成的噪音。

![打分标准](/assets/img/mini_skyvern/dafenbiaozhun.jpg){: width="100%" }

打分标准
{: style="text-align: center"}

以此，我们选择出合适的选择数据结构块放入容器当中。

>后续 AI 还自己多加了两条鲁棒性增强的兜底1内容。个人感觉相对没什么新意，也没什么好总结的，所以先跳过不写了。

#### 内容指纹

截止目前为止，我们已经理顺了对于单页浏览器进行元素收集和数据提取的工作。但是在实际情境当中，例如百度这样的浏览器上，每个搜索结果存在多页结果。所以每一页都要进行截图提取，然后翻页，重复这个步骤直到找到需要的答案/token或翻页次数等达到阈值。

但是每步都进行提取存在对同一页进行反复提取，反复烧token的问题。直觉上进行URL查重来解决这个问题可行，但是面对像无限滚动类型的网页或者只进行局部刷新的网页类型时则容易出现问题。所以有必要对于这个流程进行鲁棒性加强。

所以AI给出的解决方案是通过DOM计算来为每个网页计算一个指纹。每个页面的指纹由以下四个信号组成：
...
最后将这个指纹与URL拼接在一起，来判断页面是否一直在更新。

>为了防止 Agent 因拿不到想要的搜索结果而反复滚动截屏，AI 设计的时候通过`ExtractionAccumulator.py`这个文件来统计滚动截屏的次数并设置了12页的累积截屏上限，防止在这个过程中直接把token烧穿了。

#### 缓存脚本

缓存脚本相关的内容在文章前面的部分有涉及过，在此处再进行一个整体总结。

首先，`ScriptGenerator` 把动作轨迹固化成一份独立的 Playwright 脚本。定位用的是每个动作执行前由 agent 用 ID 反查出来的真实 XPath，因此脚本脱离标注引擎也能跑 —— 回放时页面上没有ID，XPath 是唯一定位载体。相同任务再来时命中缓存直接回放，极大提升效率

此外，缓存的回放路径与 Agent 初始路径的格式始终保持一致，所以去重、预算、核对、落库全套语义可以做到格式全部相符 —— 无论走模型决策还是走脚本回放，用户拿到的数据格式与完整度承诺是同一套。

>在缓存操作流程与数据的过程当中，需要调用勾子函数的位置要注意。在debug过程当中，发现起始goto函数的位置没有放勾子，导致每次的提取元素都会缺漏首页的内容。
...
至此，在 mini_skyvern project 中单步quest的相关内容基本上全部写完了。因为水平不足以及一边写一边学+复习的缘故，回看前面的内容，感到有些混乱。文中罗列了大量的技术实现细节，但是结构并不很清晰。虽然又开了一篇 mini_skyvern project 框架概览，还是希望下一篇技术复盘能够把架构和技术串在一起写得更有条理一些，而不是写出一份只有自己能够理解的文案。
...
就我个人的体验而言，我们需要做的是在任务开始的时候尽量设计一个优秀的框架，将需求用技术性和实用性的语言表达出来，减少模型的逻辑模糊点（否则后期处理起来较为痛苦）；在任务中面对AI在大范围铺开进行尝试的时候提供一些人类的小直觉来节省整体token数量的消耗；以及在任务结束后，根据具体的使用感受给AI明确的反馈。学习和复盘的工作放在任务已经迭代完成后进行，将AI在实际生产中产生的知识和内容内化，以提升个人在下一轮实操中的表现。
