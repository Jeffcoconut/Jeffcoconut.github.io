...
在解析 skyvern 仓库的过程当中，我们的关注点仍将聚焦于 Agent 决策层、浏览引擎层对应的部分，以提升 mini_skyvern 在核心业务操作逻辑上的性能。

通过 deepwiki 的 codemap 功能，我们可以初步发现 skyvern 最体现工程师直觉和经验的部分主要在 *网页的提取识别和处理*、 *Token预算管理* 以及应用场景的 *边界的优化处理* 上，我们接下来针对 mini_skyvern 的调整也将主要针对这三个部分展开。

>当然，skyvern还有些我们难以企及的优势：比如skyvern的API接口调的是OpenAI的某款老模型，能采用这种性能较为优秀的多模态模型对这类项目还是非常关键的。但是本复刻项目预算有限，将采用更为便宜的替代模型。

### 网页提取与处理

在进行调整优化的过程中，对 ActionType 的类型进行扩展大概是最~~无需思考和犹豫~~就进行的一项调整。 miniskyvern 的基础 Action 功能确实有些少，且调整 ActionType 对于整个项目结构没影响。最后，我们补充进了`scroll` 、`keypress`等操作，与 skyvern 仓库中的动作类型完全对齐。(对于覆盖的可交互元素类型识别，同理和 skyvern 原框架进行对齐)
...
写在最后
>虽然写一篇技术文档来复盘做过的项目还是花费相当的时间，但是沉下心来认真比对和学习，确实能找到原本的小项目中明显需要优化一些的地方，并给agent的性能和使用感觉带来实打实的提升。那闲话少叙，改mini_skyvern去了。