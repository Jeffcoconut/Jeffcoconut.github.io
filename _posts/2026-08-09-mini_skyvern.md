...
>接下来的分析中可能将着重讨论每个部分采用了哪些方法，还有哪些地方可以进行优化尝试进行研究，所以将不会把每个层内的所有内容全部面面俱到进行分析。(比如前端设计相关内容可能会被跳过)
{: .prompt-warning}
...
以下的两段内容将直接由AI生成 （~~AI的表达方法还是比个人简洁清晰很多的 而且能结合大量的代码实例进行说明 这么看我就比较废了 笑~~）
...
![DOM_tree](/assets/img/mini_skyvern/DOM_tree.png){: width="80%" }
